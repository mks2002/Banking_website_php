...
<head>
   ...
   <title>MY Bank-website | Account Open</title>
...
<body>
   <nav class="nav nav_top">
      ...
      <div class="nav_side">
         ...
         <a href="signup.php" class="active"> Account Open</a>
      </div>
   </nav>
   ...
   <div class="signup_main">
      ...
      $userid = $_POST['name'];
      $adr = $_POST['firstname1'];
      $city = $_POST['firstname2'];

      $sql = "insert into customer values ('$userid','$adr','$city')";
      $res = mysqli_query($conn, $sql);


      if ($res) {

         header("location:account_detail.php?uid=$userid");
      } else {
         if (mysqli_error($conn) == "Duplicate entry '$userid' for key 'PRIMARY'") {
            echo "<h2>This Username already exists</h2>."

      ?>
            <form action="backend2.php" method="post">

               <input type="text" name="name" id="firstname1" placeholder="enter username" required/>

               <input type="text" name="firstname1" id="firstname2" placeholder="enter adress"required />

               <input type="text" name="firstname2" id="firstname" placeholder="enter city"required />
